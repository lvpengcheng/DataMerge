...
def _filter_visible_sheets(sheets, skip_hidden: bool):
    """从 Aspose 工作表列表中过滤掉隐藏 sheet（VisibilityType != Visible）

    Args:
        sheets: Aspose 工作表对象列表
        skip_hidden: True=跳过隐藏sheet，False=保留全部
    Returns:
        过滤后的工作表列表
    """
    if not skip_hidden:
    ...
    for ws in sheets:
        try:
            if ws.IsVisible:
                ...
                logger.info(f"跳过隐藏sheet: '{ws.Name}'")
        except Exception:
            visible.append(ws)
    return visible
...
class IntelligentExcelParser:
    ...
    def parse_excel_file(self, file_path: str, max_data_rows: int = None, skip_rows: int = 0,
        ...
        try:
            ...
            # 确定要处理的 sheet 列表
            _skip_hidden = _should_skip_hidden_sheets() if skip_hidden_sheets is None else skip_hidden_sheets
            if active_sheet_only:
                active_idx = aspose_wb.Worksheets.ActiveSheetIndex
                sheets_to_parse = [aspose_wb.Worksheets[active_idx]]
                # 激活sheet本身就是隐藏的话仍然解析（用户显式指定）
            else:
                all_sheets = [aspose_wb.Worksheets[i] for i in range(aspose_wb.Worksheets.Count)]
                sheets_to_parse = _filter_visible_sheets(all_sheets, _skip_hidden)
    ...
    def _looks_like_data_row_keys(keys: List[str]) -> bool:
        ...
        - ID 码键（如 MNCN00733）
        - 日期/年月模式
        - 三段及以上 "X-Y-Z" 拼接，且每段都包含数字（避免 YYYY-MM-DD 等格式占位符）

        需要 ≥2 个不同 key 命中才返回 True，避免单 key 偶发性误判。
        """
        if not keys:
            return False
        triggered = set()
        for k in keys:
            ...
            if re.search(r'\d{4}[年/.\-]\d{1,2}|\d{1,2}\.\d{1,2}\s*[-~至]', s):
                triggered.add(s); continue
            m = re.search(r'([\u4e00-\u9fff\w]+)-([\u4e00-\u9fff\w]+)-([\u4e00-\u9fff\w]+)', s)
            if m and all(re.search(r'\d', g) for g in m.groups()):
                triggered.add(s); continue
    ...
    def _normalize_sheet_regions(self, regions: List[ExcelRegion]) -> None:
        ...
        """
        # 【剪除退化区域】孤儿单行 / 全 Column_* 幻影 / 仅有 1 行数据的伪头
        pruned = []
        for r in regions:
            keys = list(r.head_data.keys()) if r.head_data else []
            no_data = (r.data_row_start == 0 and r.data_row_end == 0)
            data_rows = (r.data_row_end - r.data_row_start + 1) if r.data_row_start > 0 else 0

            is_orphan = no_data and self._looks_like_data_row_keys(keys)
            is_phantom = bool(keys) and all(k.startswith('Column_') for k in keys)
            # 伪头(_is_fake_merged_header)代表整个 head 是退化结构(全占位/合并展开),
            # 数据再多也不是真正的表格,直接剪除
            is_degen_fake = self._is_fake_merged_header(r.head_data)

            if is_orphan or is_phantom or is_degen_fake:
                self.logger.info(
                    f"剪除退化区域 head {r.head_row_start}-{r.head_row_end} "
                    f"data {r.data_row_start}-{r.data_row_end}"
                )
                continue
            pruned.append(r)
        regions[:] = pruned
    ...
    def _find_header_start(self, worksheet: Any, header_end_row: int, min_row: int, max_col: int) -> int:
        ...
        for row in range(header_end_row - 1, max(min_row - 1, 0), -1):
            ...
            # 【Fix D】稀疏分组行：上层 ≥2 个文本格、且全部位置是下层非空列的子集
            #   场景：r11=['','','','','','税前','税前','税前',...] 在 r12=字段名上方
            elif features.text_ratio >= 0.8 and features.non_empty_count >= 2:
                upper_cols = self._row_non_empty_cols(worksheet, row, max_col)
                lower_cols = self._row_non_empty_cols(worksheet, prev_row, max_col)
                if upper_cols and upper_cols.issubset(lower_cols):
                    should_continue = True
    ...
    def _row_non_empty_cols(self, worksheet: Any, row: int, max_col: int) -> set:
        """返回该行非空单元格的列号集合"""
        cols = set()
    ...
    def _find_header_end(self, worksheet: Any, start_row: int, max_row: int, max_col: int) -> int:
        ...
        # 【Fix E】数据透视表多行表头：start_row 含 '列标签'/'行标签' → 向下吞并 pivot 关键字行
        pivot_keywords = ('求和项', '计数项', '平均值', '最大值', '最小值', '行标签', '列标签')

        def _row_text(r: int) -> str:
            return ' '.join(
                str(worksheet.cell(r, c).value or '') for c in range(1, max_col + 1)
            )

        start_text = _row_text(start_row)
        if '列标签' in start_text or '行标签' in start_text:
            r = start_row + 1
            while r <= min(start_row + MAX_HEADER_ROWS, max_row):
                if self._is_empty_row(worksheet, r, max_col):
                    break
                row_text = _row_text(r)
                has_pivot = any(kw in row_text for kw in pivot_keywords)
                # 只有含 pivot 关键字的行才视作表头延伸；否则停止（避免吞数据行）
                if has_pivot:
                    current_header_end = r
                    r += 1
                    continue
                break
    ...
    def _has_data_continuation_after_gap(self, worksheet: Any, current_row: int,
        ...
        用于支持「数据末尾有空行间隔再出现真实数据」的稀疏数据场景。
        """
        LOOKAHEAD = 20
        look_limit = min(current_row + LOOKAHEAD, max_row)
        for r in range(current_row + 1, look_limit + 1):
            if self._is_empty_row(worksheet, r, max_col):
                continue
            # 找到首个非空行
            ...
            if self._is_summary_row(worksheet, r, max_col):
                return False
            feats = self.row_analyzer.analyze_row_features(worksheet, r, max_col)
            if start_features.non_empty_count > 0 and feats.non_empty_count > 0:
                hi = max(feats.non_empty_count, start_features.non_empty_count)
                lo = min(feats.non_empty_count, start_features.non_empty_count)
    ...
    def _is_title_row(self, worksheet: Any, row: int, max_col: int) -> bool:
        ...
        # 情况1c/1d 兜底：
        # 因为 Aspose 在合并单元格内会把同一值"展开"到多列，单个标题文本会被算作多个非空。
        # 1c (日期模式) 仍允许 1-2 个非空,因为日期 banner 可能拆成两格;
        # 1d (长文本横幅) 仅限单 cell 或合并展开的横幅,避免把 2 列真实表头(如
        # ['新成本中心编号','新成本中心名称'])或 2 列重复值的真实数据行(如
        ...
        # 用于捕获 "On board in Apr."、"苏日娜社保个人部分垫付" 等单 cell 横幅
        if non_empty_cells == 1 or (is_merged_banner and non_empty_cells >= 3):
            if len(first_value_str) >= 6 and first_value_str not in self.HEADER_KEYWORDS:
                if not (first_value_str.endswith(':') or first_value_str.endswith('：')):
                    if not any(m in first_value_str for m in self.REQUIRED_FIELD_MARKERS):
                        return True
    ...
    def merge_source_to_target(self, source_folder: str, expected_file: str,
        ...
        for file_path in sorted(source_folder.iterdir()):
            ...
            try:
                ...
                if active_sheet_only:
                    ...
                    # 每个sheet各自取最优区域（默认跳过隐藏sheet）
                    _skip_hidden = _should_skip_hidden_sheets()
                    all_sheets = [aspose_wb.Worksheets[i] for i in range(aspose_wb.Worksheets.Count)]
                    visible_sheets = _filter_visible_sheets(all_sheets, _skip_hidden)
                    for aspose_ws in visible_sheets:
                        ws = _AsposeWorksheet(aspose_ws)
                        sheet_data = self._parse_sheet(ws, max_data_rows=2)
                        if sheet_data and sheet_data.regions:
                            _, regions = self._find_valid_region([sheet_data])
        ...
        if active_sheet_only:
            ...
            _skip_hidden = _should_skip_hidden_sheets()
            all_exp_sheets = [expected_wb.Worksheets[i]
                              for i in range(expected_wb.Worksheets.Count)]
            exp_sheets = _filter_visible_sheets(all_exp_sheets, _skip_hidden)
