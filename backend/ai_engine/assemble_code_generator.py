...
class AssembleCodeGenerator(TemplateCodeGenerator):
    ...
    def _build_prompt(
        ...
        parts.append(
            ...
            "每表取对应列汇总进结果表。\n"
            "模式3（混合/两阶段）：同时存在同构子集与职责表（如上海/北京/南京同构表 + "
            "离职/入职/变更职责表）→ 策略分两阶段：\n"
            "  阶段1（同构合并）：对同构的那部分表【纵向拼接】→ 按主键数值加总/文本取首值去重"
            "→ 生成中间结果（列 = 参与表的最大列并集，程序自建，不依赖模板）；\n"
            "  阶段2（关联汇总）：中间结果 + 其余职责表 → 按主键【关联合并】→ 最终写入模板数据行。\n"
            "中间态用 pandas DataFrame（concat + groupby + merge），不必创建中间 Excel sheet。\n"
            "判断依据：全部源表列集合高度重合 → 模式1；各有专责列 → 模式2；"
            "部分同构 + 部分专责 → 模式3。拿不准时按模式2处理，并在 report 里说明判断。"
        )
        ...
        parts.append(
            ...
            "必须额外输出结构化字段映射，供人工复核。每个目标列的值是【条目列表】"
            "（一个目标列可对应多个源表，同构整合模式下一对多；职责分工模式下列表只有一个元素）：\n"
            "FIELD_MAPPING = {\n"
            "    \"目标列名\": [\n"
            "        {\"source_sheet\": \"源_表名\", \"source_column\": \"源列名\", "
